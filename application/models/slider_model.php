<?php 
    class Slider_model extends CI_Model {
        ...
        public function create_slider($data,$img){
            if(!empty($data['yt_link'])){
                $yt = $this->after('https://youtu.be/',$data['yt_link']);
                $data = array(
                    'post_title' => $data['title'] , 
                    'post_description' => $data['caption'],
                    'slider_order' =>$data['order'],
        ...
        public function fetch_slider(){
            ...
            $term_title = 'slider';
            $term_id = 0 ;
            $sql = 'select post_title,post_description,post_img,post.post_id,status,yt_link FROM post 
        ...
        public function fetch_slider_info($news_id){
            $active = 1;
            $sql = 'SELECT post_title,post_description,post_img,post_id,slider_order,yt_link FROM post WHERE post_id =:post_id and active = :active ';
            $stmt = $this->db->conn_id->prepare($sql);
        ...
        public function after($word, $inthat)
        {
