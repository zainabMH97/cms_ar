    ...
    class News_model extends CI_Model {
        ...
        public function create_news($news,$img,$terms){
                $data = array(
                    ...
                    'post_slug' => urlencode($news['post_slug']),
        ...
        public function fetch_uncategorieznews($limit,$offset){
            ...
            WHERE
                term_relation.term_id IS NULL and post.active = :active limit :offset , :limit ';
            $stmt = $this->db->conn_id->prepare($sql);
            $stmt->execute([':offset' => $offset,':limit'=>$limit,':active'=>$active]);
            $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
            if($row){
                $result = [];
                foreach($row as $r){
        ...
        public function countNewsUncategorized(){
            $query = $this->db->query('SELECT post.post_id FROM post  LEFT JOIN term_relation ON post.post_id = term_relation.post_id
                WHERE term_relation.term_id IS NULL and post.active = 1');
                return $query->num_rows();
        }
