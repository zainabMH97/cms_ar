<?php 
    class Search_model extends CI_Model {
        ...
        public function fetch_data_viewNews($id){
            $status = 'publish';
            $active = 1;
           $sql = 'SELECT post_title,post_description,post_img,post_date,post_slug,status,post_date FROM post inner join term_relation on post.post_id = term_relation.post_id where term_relation.term_id = :term_id and post.active = :active';
           $stmt = $this->db->conn_id->prepare($sql);
           $stmt->execute([':active'=>$active,':term_id'=>$id]);
           $row = $stmt->fetchAll(PDO::FETCH_ASSOC);
           if($row){
               $result = [];
               foreach($row as $r){
