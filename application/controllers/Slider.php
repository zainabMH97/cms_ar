    ...
    public function createSlider(){
        if(isLoggedIn()){
            if($this->session->userdata['admin_type'] === 'main_admin'){
                ...
                if($ext == 'mp4'){
                    setFlashData('alert-danger','This Media Type Is Not Accapted','slider/addSlider');
                }else{
                    ...
                    $data['yt_link'] = clean_input($this->input->post('yt_link'));
                    $data['caption'] = clean_input($this->input->post('caption'));
                    $data['order'] = clean_input($this->input->post('order'));
    ...
    public function updateSlider(){
        if(isLoggedIn()){
            if($this->session->userdata['admin_type'] === 'main_admin'){
                ...
                $ext = pathinfo($path, PATHINFO_EXTENSION);
                if($ext == 'mp4'){
                    setFlashData('alert-danger','This Media Type Is Not Accapted','slider/editSlider');
                }else{
                    if(empty($_FILES['userfile']['name'])){
                        $post_image = Null;
                    }else{
